...
void est_forcing_matrix_benney(double **L) {
  // transpose of L
  double **Lt = malloc_f2d(P, N);

  // transpose of the system matrix
  double **At = malloc_f2d(N, N);
  benney_jacobian(At);
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      double tmp = At[i][j];
 ...
 * main problem */
void est_gain_matrix_benney(double **K) {
  /* Jacobian */
  ...
  /* full control matrix */
  dlqr(A, B, sqrt(DX) * MU, 1.0 / sqrt(DX), N, M, K);
...
int est_update_benney(double dt, double *H) {
  static double t = 0.0;
  t += dt;
  ...
  return benney_step(&data, dt, 1.0e-6, 100);
}


/* solve the (transpose) LQR problem to compute L. This requires EST_C to have
 * been filled with the Benney or WR observer. */
void est_forcing_matrix_wr(double **L) {
  // transpose of L
...
/* compute the gain matrix mapping the estimator to actuator strengths for the
 * main problem */
void est_gain_matrix_wr(double **K) {
  /* Jacobian */
...
/* step the estimator forward in time using observations of the real height H.
 * Returns the number of iterations used. */
int est_update_wr(double dt, double *H) {
  // don't bother doing anything if EST_h < 0.0 or is nan
  for (int i = 0; i < N; i++) {
    if (EST_h[i] < 0.0 || isnan(EST_h[i])) {
      return 0;
...
void est_set(void) {
  ...
  case BENNEY:
    ...
    EST_L = malloc_f2d(N, P);
    est_forcing_matrix_benney(EST_L);

    // main forcing matrix
    EST_K = malloc_f2d(M, N);
    est_gain_matrix_benney(EST_K);

    // estimator forcing term
    EST_f = malloc(N * sizeof(double));

    // workspace for solvers
    EST_work = malloc(8 * N * sizeof(double));
    break;
  case WR:
    // estimator forcing matrix
    EST_L = malloc_f2d(2 * N, P);
    est_forcing_matrix_wr(EST_L);

    // main forcing matrix
    EST_K = malloc_f2d(M, 2 * N);
    est_gain_matrix_wr(EST_K);
...
int est_step(double dt, double *h, double *q, int control_on) {
  ...
  if (RT == WR) {
    return est_update_wr(dt, h);
  }
  if (RT == BENNEY) {
    return est_update_benney(dt, h);
  }
...
void est_output(void) {
  ...
  if (RT == BENNEY) {
    output_d2d("out/L.dat", EST_L, N, P);
    output_d2d("out/K.dat", EST_K, M, N);
    return;
  }
